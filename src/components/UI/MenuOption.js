...
const StyledMenuOption = styled.a`
  ...
  &:after {
    ...
    border-bottom: 1px solid
      ${(props) => (props.theme.isDark ? props.theme.baseColors.dark : props.theme.baseColors.middleLight)};
    bottom: 1px;
    left: 10px;
    right: 10px;
  }
  ...
  &:hover {
    ${(props) =>
      !!!props.option.disabled
        ? css`
            color: ${props.theme.accentColors.primary.color};
          `
        : null};
...
const MenuOption = (props) => {
  ...
  return (
    <StyledMenuOption className={className} option={option} {...dynamicProps} onClick={_onClick}>
      {option ? option.label : ''}
    </StyledMenuOption>
  );
};
